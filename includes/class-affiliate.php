	...
	public function list( array $args = array() ): array {
		...
		if ( $status && in_array( $status, $this->statuses(), true ) ) {
			return $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$this->table()} WHERE status = %s ORDER BY date_created DESC LIMIT %d OFFSET %d",
					$status,
					$limit,
					$offset
				)
		...
		return $wpdb->get_results(
			$wpdb->prepare( "SELECT * FROM {$this->table()} ORDER BY date_created DESC LIMIT %d OFFSET %d", $limit, $offset )
		);
	}
